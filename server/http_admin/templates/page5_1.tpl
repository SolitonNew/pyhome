<script type="text/javascript">
    ...
    var date_intervals = [6, 12, 24, 24 * 3, 24 * 7, 24 * 14, 24 * 30, 24 * 90, 24 * 30 * 6, 24 * 30 * 12];
    var stat_panel_start = {{ widget('START_TIME') }};
    var stat_panels = [];
    var stat_panels_ids = [];
    ...
    function append_stat_panel() {
        $.ajax({url:'page5_1?FORM_QUERY=append_panel'}).done(function (data) {            
    ...
    function del_stat_panel(key) {
        $.ajax({url:'page5_1?FORM_QUERY=del_panel&key=' + key}).done(function (data) {
            if (data == 'OK') {
