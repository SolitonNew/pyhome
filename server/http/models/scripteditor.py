...
class ScriptEditor(BaseForm):
    ...
    def create_widgets(self):
        text = ""
        for row in self.db.select("select DATA from core_scripts where ID = %s" % self.param('key')):
    ...
    def query(self, query_type):
        if query_type == "save":
            try:
                ...
                self.db.IUD("update core_scripts set DATA = %s where ID=%s", (data, self.param('key')))
                self.db.commit()
